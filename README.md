...
```text
https://<LAN-IP-Raspberry>:8443
```

Доступ дополнительно разрешён только клиентам той же проводной IPv4-подсети. Панель не слушает адрес `wlan0`, не загружает внешние JavaScript/CSS-библиотеки и не доступна клиентам точки доступа. Сертификат самоподписанный, поэтому при первом открытии браузер покажет предупреждение.

В панели доступны:

- краткое состояние системных сервисов и текущий внешний IP;
- безопасная смена VPS с проверкой fingerprint и тестовым SOCKS до переключения;
- изменение SSID, WiFi-пароля, страны и канала;
- ручной перезапуск SSH-туннеля.

Перед изменением VPS или WiFi создаётся резервная копия. Пароли никогда не возвращаются браузеру и не записываются в веб-журнал.
